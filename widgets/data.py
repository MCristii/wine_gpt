...
def insert_wine(data: pd.DataFrame) -> None:
    ...
    grape_variety = st.multiselect("Grape Variety", set([*chain.from_iterable(data['grape_variety'])]))
    year = st.number_input("Year", 1950, datetime.now().year)
    price = st.slider("Price", 15, 1000)
    wine_type = st.selectbox("Type", ['Brut', 'Extra-Dry', 'Dry', 'Semi-Dry', 'Semi-Sweet', 'Sweet'])
